...
import com.example.kotlin_rxjava.utils.toAge
import com.example.kotlin_rxjava.utils.toStandardDateFormat
        ...
        aboutViewModel.cast.observe(viewLifecycleOwner) {
            when(it.status) {
                Status.SUCCESS -> {
                    it.data?.let { cast ->
                        tvBirthday.text =
                            if ( !cast.birthday?.toStandardDateFormat().isNullOrEmpty() )
                        ...
                        tvKnownAs.text =
                            if ( !cast.knownAs.isNullOrEmpty() ) cast.knownAs.joinToString(", ") else getString(R.string.dash)
